...
ZENCHAIN_HOME="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORK="$(mktemp -d -t zenchain-test.XXXXXX)"
trap 'rm -rf "$WORK"' EXIT
fail=0

bash -n "$ZENCHAIN_HOME/zen"           || { echo "FAIL  zen has a syntax error"; exit 1; }
if [[ -x "$ZENCHAIN_HOME/zen" ]]; then
  bash -n "$ZENCHAIN_HOME/zen" || { echo "FAIL  zen has a syntax error"; exit 1; }
else
  echo "FAIL  zen executable source missing"; fail=1
fi
[[ ! -e "$ZENCHAIN_HOME/orc2" ]] \
  || { echo "FAIL  legacy root orc2 executable still present"; fail=1; }
if "$ZENCHAIN_HOME/zen" help 2>&1 | grep -q '^ZenChain'; then
  echo "PASS  public product name is ZenChain"
else
  echo "FAIL  public product name is not ZenChain"; fail=1
fi
if "$ZENCHAIN_HOME/zen" help 2>&1 | grep -q 'zen init'; then
  echo "PASS  public executable examples use zen"
else
  echo "FAIL  public executable examples do not use zen"; fail=1
fi
if grep -q '^# ZenChain' "$ZENCHAIN_HOME/README.md" && grep -q 'zen init' "$ZENCHAIN_HOME/README.md"; then
...
if [[ -f "$ZENCHAIN_HOME/bin/zenchain-anti-slop-check" ]]; then
  ...
  echo "FAIL  zenchain-anti-slop-check source missing"; fail=1
fi
...
combos=(
  "defaults|"
  "figma-pi-2lane-postgres-codegen-slack|ZENCHAIN_TRACKER=scratch ZENCHAIN_DESIGN=figma ZENCHAIN_RUNNER=claude ZENCHAIN_RUNNER_MECH=pi ZENCHAIN_LANES=2 ZENCHAIN_DB=postgres ZENCHAIN_CODEGEN=yes ZENCHAIN_NOTIFY=slack"
  "github-lofi-codex-1lane-sqlite-push|ZENCHAIN_TRACKER=github ZENCHAIN_DESIGN=lofi ZENCHAIN_RUNNER=codex ZENCHAIN_RUNNER_MECH=codex ZENCHAIN_LANES=1 ZENCHAIN_DB=sqlite ZENCHAIN_CODEGEN=no ZENCHAIN_NOTIFY=push"
  "github-none-claude-2lane-nodb|ZENCHAIN_TRACKER=github ZENCHAIN_DESIGN=none ZENCHAIN_RUNNER=claude ZENCHAIN_RUNNER_MECH=claude ZENCHAIN_LANES=2 ZENCHAIN_DB=none ZENCHAIN_CODEGEN=yes ZENCHAIN_NOTIFY=none"
  "scratch-figma-codexmech-2lane-sqlite|ZENCHAIN_TRACKER=scratch ZENCHAIN_DESIGN=figma ZENCHAIN_RUNNER=claude ZENCHAIN_RUNNER_MECH=codex ZENCHAIN_LANES=2 ZENCHAIN_DB=sqlite ZENCHAIN_CODEGEN=no ZENCHAIN_NOTIFY=slack"
  "legacy-yesno-db-still-renders|ZENCHAIN_TRACKER=scratch ZENCHAIN_DESIGN=none ZENCHAIN_LANES=2 ZENCHAIN_DB=yes ZENCHAIN_CODEGEN=no"
  # Engine set, dependent values omitted — must derive from the project name,
  # never render an empty guard.
  "db-set-without-names|ZENCHAIN_DB=postgres ZENCHAIN_NOTIFY=slack ZENCHAIN_LANES=2"
  ...
  "my-zebra-shop|ZENCHAIN_DB=postgres ZENCHAIN_LANES=2"
  "sqlite-set-without-names|ZENCHAIN_DB=sqlite ZENCHAIN_LANES=2"
  "anti-slop-off|ZENCHAIN_ANTI_SLOP=off"
)
...
for combo in "${combos[@]}"; do
  ...
  if ! "$ZENCHAIN_HOME/zen" init "$dir" --answers "$env_file" >"$WORK/$name.log" 2>&1; then
    echo "FAIL  $name — init exited non-zero"; sed 's/^/      /' "$WORK/$name.log"; fail=1; continue
  fi

  bad="$(grep -rl '{{\|MISSING FRAGMENT\|zenchain:include' "$dir" --include='*.md' 2>/dev/null || true)"
  if [[ -n "$bad" ]]; then
  ...
  for role in implementer reviewer fixer qa decider explorer; do
    [[ -f "$dir/.claude/agents/$role.md" || -f "$dir/.zenchain/agents/$role.md" ]] \
      || { echo "FAIL  $name — role $role not rendered"; fail=1; }
  done
  [[ -x "$dir/.zenchain/bin/zenchain-ticket-check" ]] \
  ...
  anti_slop="$(sed -n 's/^ZENCHAIN_ANTI_SLOP="\{0,1\}\([a-z]*\)"\{0,1\}$/\1/p' "$dir/.zenchain/config.env" | head -1)"
  ...
  [[ -x "$dir/.zenchain/bin/zenchain-anti-slop-check" ]] \
    || { echo "FAIL  $name — anti-slop gate checker not rendered executable"; fail=1; }
  orch="$(ls "$dir"/{.zenchain,docs/pipeline}/ORCHESTRATOR.md 2>/dev/null | head -1 || true)"
  if [[ "$anti_slop" == enforced ]]; then
    grep -q '^\.zenchain/bin/zenchain-anti-slop-check$' "$orch" \
  ...
  grep -q 'You run the gate, not the agents' "$dir"/{.zenchain,docs/pipeline}/ORCHESTRATOR.md 2>/dev/null \
  ...
  db="$(sed -n 's/^ZENCHAIN_DB="\{0,1\}\([a-z]*\)"\{0,1\}$/\1/p' "$dir/.zenchain/config.env" | head -1)"
  if [[ -n "$db" && "$db" != none ]]; then
    ...
    if [[ "$db" == postgres ]]; then
      for k in ZENCHAIN_DEV_DB ZENCHAIN_LANE_PREFIX; do
        val="$(sed -n "s/^$k=\"\{0,1\}\([^\"]*\)\"\{0,1\}\$/\1/p" "$dir/.zenchain/config.env" | head -1)"
        [[ "$val" =~ ^[a-z_][a-z0-9_]*$ ]] \
  ...
  sk="$dir/.claude/skills"; [[ -d "$dir/.zenchain/skills" ]] && sk="$dir/.zenchain/skills"
  if [[ -d "$sk" ]]; then
    ...
    grep -q '^ZENCHAIN_SKILLS_PIN="[a-f0-9]\{40\}"$' "$dir/.zenchain/config.env" \
  ...
  rv="$dir/.claude/agents/reviewer.md"; [[ -f "$rv" ]] || rv="$dir/.zenchain/agents/reviewer.md"
  ...
  im="$dir/.claude/agents/implementer.md"; [[ -f "$im" ]] || im="$dir/.zenchain/agents/implementer.md"
  ...
  fx="$dir/.claude/agents/fixer.md"; [[ -f "$fx" ]] || fx="$dir/.zenchain/agents/fixer.md"
  ...
  # was the original bug: `zenchain init` printed /run-prd on an empty repo, where
  # nothing had yet decided what to build.
  ns="$("$ZENCHAIN_HOME/zen" doctor "$dir" 2>&1 || true)"
  ...
  blocks="$( { grep -rc 'zenchain:agent-skills:start' "$dir"/{CLAUDE.md,AGENTS.md} 2>/dev/null || true; } | awk -F: '{s+=$2} END{print s+0}')"
  [[ "$blocks" == 1 ]] || { echo "FAIL  $name — Agent skills block appears $blocks times, expected 1"; fail=1; }
  grep -q 'zenchain:agent-skills:start' "$dir/AGENTS.md" 2>/dev/null \
  ...
  for role in implementer fixer reviewer; do
    rf="$dir/.claude/agents/$role.md"; [[ -f "$rf" ]] || rf="$dir/.zenchain/agents/$role.md"
  ...
  grep -q 'Approved Technical Changes' "$dir"/{.claude,.zenchain}/agents/reviewer.md 2>/dev/null \
    || { echo "FAIL  $name — reviewer missing issue-authorised technical changes"; fail=1; }
  grep -q 'You do not choose dependencies' "$dir"/{.claude,.zenchain}/agents/implementer.md 2>/dev/null \
  ...
  dc="$dir/.claude/agents/decider.md"; [[ -f "$dc" ]] || dc="$dir/.zenchain/agents/decider.md"
  ...
  cp -R "$dir" "$dir.first"
  "$ZENCHAIN_HOME/zen" render "$dir" >/dev/null 2>&1 || { echo "FAIL  $name — render round trip exited non-zero"; fail=1; }
...
checker="$WORK/defaults/.zenchain/bin/zenchain-ticket-check"
...
anti_checker="$ZENCHAIN_HOME/bin/zenchain-anti-slop-check"
if [[ ! -x "$anti_checker" ]]; then
  echo "FAIL  anti-slop behavior checks unavailable — source checker missing"; fail=1
else
  d="$WORK/anti-slop-valid"; mkdir -p "$d/.zenchain" "$d/tools/oxlint/anti-slop"
...
RULES
  ...
  printf '%s\n' 'ZENCHAIN_ANTI_SLOP="enforced"' 'ZENCHAIN_GATE="npm run lint; npm test"' >"$d/.zenchain/config.env"
  ...
  d="$WORK/anti-slop-missing"; mkdir -p "$d/.zenchain"
  printf '%s\n' '{"devDependencies":{"oxlint":"1.78.0","@oxlint/plugins":"1.78.0"}}' >"$d/package.json"
  cp "$WORK/anti-slop-valid/oxlint.config.ts" "$d/oxlint.config.ts"
  printf '%s\n' 'ZENCHAIN_ANTI_SLOP="enforced"' 'ZENCHAIN_GATE="npm test"' >"$d/.zenchain/config.env"
  ...
  d="$WORK/anti-slop-effect"; mkdir -p "$d/.zenchain" "$d/tools/oxlint/anti-slop"
  printf '%s\n' '{"dependencies":{"effect":"3.0.0"},"devDependencies":{"oxlint":"1.78.0","@oxlint/plugins":"1.78.0"}}' >"$d/package.json"
  cp "$WORK/anti-slop-valid/oxlint.config.ts" "$d/oxlint.config.ts"
  printf '%s\n' 'export const antiSlop = true' >"$d/tools/oxlint/anti-slop/index.ts"
  printf '%s\n' 'ZENCHAIN_ANTI_SLOP="enforced"' 'ZENCHAIN_GATE="npm run lint"' >"$d/.zenchain/config.env"
  ...
  d="$WORK/anti-slop-non-js"; mkdir -p "$d/.zenchain"
  printf '%s\n' 'ZENCHAIN_ANTI_SLOP="enforced"' 'ZENCHAIN_GATE="cargo test"' >"$d/.zenchain/config.env"
  "$anti_checker" "$d" >/dev/null 2>&1 \
    && echo "PASS  anti-slop leaves non-JS repositories unaffected" \
    || { echo "FAIL  anti-slop blocked a non-JS repository"; fail=1; }

  d="$WORK/anti-slop-disabled"; mkdir -p "$d/.zenchain"
  printf '%s\n' '{"devDependencies":{}}' >"$d/package.json"
  printf '%s\n' 'ZENCHAIN_ANTI_SLOP="off"' 'ZENCHAIN_GATE="npm test"' >"$d/.zenchain/config.env"
  "$anti_checker" "$d" >/dev/null 2>&1 \
...
"$ZENCHAIN_HOME/zen" init "$d" --yes >/dev/null 2>&1
printf '\n## Refund while offline\n- Answer: pending\n' >>"$d/docs/INBOX.md"
before="$(shasum -a 256 "$d/docs/INBOX.md" | awk '{print $1}')"
"$ZENCHAIN_HOME/zen" render "$d" >/dev/null 2>&1
...
printf 'ZENCHAIN_PROJECT="legacy"\nZENCHAIN_TRACKER="scratch"\n' >"$d/.zenchain-config.env"
mkdir -p "$d/.zenchain"; cp "$d/.zenchain-config.env" "$d/.zenchain/config.env"
if "$ZENCHAIN_HOME/zen" render "$d" >"$WORK/legacy-inbox.log" 2>&1; then
  echo "FAIL  pending legacy INBOX did not block render"; fail=1
elif [[ -e "$d/docs/INBOX.md" || -e "$d/.zenchain/ORCHESTRATOR.md" ]]; then
...
got_sha="$(sed -n 's/^ZENCHAIN_SKILLS_PIN="\([a-f0-9]*\)"$/\1/p' "$WORK/defaults/.zenchain/config.env" | head -1 || true)"
if [[ -z "$want_sha" ]]; then
...
ln -sf "$ZENCHAIN_HOME/zen" "$WORK/zenchain-link"
lnk="$WORK/via-link"; mkdir -p "$lnk"; git -C "$lnk" init -q .
if "$WORK/zenchain-link" init "$lnk" --yes >"$WORK/via-link.log" 2>&1 \
...
cp "$ZENCHAIN_HOME/zen" "$WORK/orphan-zenchain"; chmod +x "$WORK/orphan-zenchain"
orphan="$WORK/orphan"; mkdir -p "$orphan"; git -C "$orphan" init -q .
if "$WORK/orphan-zenchain" init "$orphan" --yes >/dev/null 2>&1; then
  echo "FAIL  a copied (not symlinked) script rendered without its templates"; fail=1
elif [[ -e "$orphan/.zenchain" ]]; then
...
agentfile_case() { # agentfile_case <name> <setup-fn>
  ...
  $setup "$d"
  "$ZENCHAIN_HOME/zen" init "$d" --yes >"$d.log" 2>&1 || { echo "FAIL  agent-file/$nm — init failed"; fail=1; return; }
  local n
  n="$( { grep -rc 'zenchain:agent-skills:start' "$d"/{CLAUDE.md,AGENTS.md} 2>/dev/null || true; } | awk -F: '{s+=$2} END{print s+0}')"
  if [[ "$n" != 1 ]]; then echo "FAIL  agent-file/$nm — $n blocks, expected exactly 1"; fail=1; return; fi
  grep -q 'zenchain:agent-skills:start' "$d/AGENTS.md" 2>/dev/null || { echo "FAIL  agent-file/$nm — block not in AGENTS.md"; fail=1; return; }
...
"$ZENCHAIN_HOME/zen" render "$WORK/af-existing-claude" >/dev/null 2>&1 || true
...
af_out="$("$ZENCHAIN_HOME/zen" doctor "$WORK/af-existing-claude" 2>&1 || true)"
...
# line of a multi-line `export A B C \`, which an `^export ZENCHAIN_X` pattern misses.
# `\b` is not portable under LC_ALL=C on BSD grep, so use an explicit delimiter class.
unexported=""
for v in $(grep -rho '{{[A-Z_0-9]*}}' "$ZENCHAIN_HOME/templates/" | tr -d '{}' | sort -u); do
  grep -qE "ZENCHAIN_$v[^A-Z_0-9]" "$ZENCHAIN_HOME/zen" || unexported="$unexported $v"
done
...
printf '# app\n\n<!-- zenchain:agent-skills:start -->\nold\n\nHUMAN NOTES\n' >"$d/AGENTS.md"
"$ZENCHAIN_HOME/zen" init "$d" --yes >/dev/null 2>&1 || true
...
"$ZENCHAIN_HOME/zen" init "$d" --yes >/dev/null 2>&1
chmod 644 "$d/AGENTS.md"; "$ZENCHAIN_HOME/zen" render "$d" >/dev/null 2>&1
...
perl -pi -e 's/^ZENCHAIN_TRACKER=.*/ZENCHAIN_TRACKER="jira"/' "$d/.zenchain/config.env"
"$ZENCHAIN_HOME/zen" render "$d" >/dev/null 2>&1 || true
...
grep -q 'ZENCHAIN_SKILLS_REPO##\*/' "$ZENCHAIN_HOME/zen" \
...
grep -q '10#\$ans' "$ZENCHAIN_HOME/zen" \
...
grep -q 'n<2 && /\^---\$/' "$ZENCHAIN_HOME/bin/zenchain-agent" \
...
if grep -q 'Do not substitute a colour' "$ZENCHAIN_HOME/templates/skills/figma-to-code/SKILL.md" \
   && grep -q 'reference loses and the human picks the replacement' "$ZENCHAIN_HOME/templates/ORCHESTRATOR.md"; then
...
"$ZENCHAIN_HOME/zen" init "$d" --yes >/dev/null 2>&1
chmod 555 "$d"
if "$ZENCHAIN_HOME/zen" render "$d" >"$d.log" 2>&1; then
...
"$ZENCHAIN_HOME/zen" init "$d" --yes >/dev/null 2>&1
if grep -qE 'ZENCHAIN_(SKILL_HOW|EXPLORER_HOW|SKILLS_DIR|DB_RECORD_LINE)=' "$d/.zenchain/config.env"; then
...
if grep -q 'if ! mv "\$src" "\$real"' "$ZENCHAIN_HOME/zen"; then
...
badmode=""
for f in .zenchain/ORCHESTRATOR.md .claude/agents/reviewer.md docs/agents/flow.md AGENTS.md; do
  [[ -e "$d/$f" ]] || continue
...
printf '# app\n\nInline zenchain:agent-skills:start and zenchain:agent-skills:end mention.\n\nTAIL NOTES\n' >"$d/AGENTS.md"
"$ZENCHAIN_HOME/zen" init "$d" --yes >/dev/null 2>&1 || true
...
if "$ZENCHAIN_HOME/zen" render "$d" >/dev/null 2>&1; then
...
printf 'ZENCHAIN_RUNNER="claude"\nZENCHAIN_RUNNER_MECH="pi"\nZENCHAIN_MODEL_BUILD="stub/model"\n' >"$d/e.env"
"$ZENCHAIN_HOME/zen" init "$d" --answers "$d/e.env" >/dev/null 2>&1
for role in implementer fixer; do
  rf="$d/.zenchain/agents/$role.md"; [[ -f "$rf" ]] || rf="$d/.claude/agents/$role.md"
...
argv_pi="$(cd "$d" && PATH="$stub:$PATH" .zenchain/bin/zenchain-agent implementer "x" 2>&1 || true)"
argv_cl="$(cd "$d" && PATH="$stub:$PATH" .zenchain/bin/zenchain-agent implementer --backend claude "x" 2>&1 || true)"
argv_cx="$(cd "$d" && PATH="$stub:$PATH" .zenchain/bin/zenchain-agent implementer --backend codex  "x" 2>&1 || true)"
bad=""
...
ZENCHAIN_UPSTREAM_SKILLS="$empty" "$ZENCHAIN_HOME/zen" init "$d" --yes >/dev/null 2>&1
sk="$d/.claude/skills"
fe_bad=""
...
ZENCHAIN_UPSTREAM_SKILLS="$fakeglobal" "$ZENCHAIN_HOME/zen" init "$d2" --yes >/dev/null 2>&1
...
d="$WORK/defaults"
if out="$("$ZENCHAIN_HOME/zen" doctor "$d" 2>&1)"; then :; fi
...
bridge="$WORK/figma-pi-2lane-postgres-codegen-slack/.zenchain/bin/zenchain-agent"
