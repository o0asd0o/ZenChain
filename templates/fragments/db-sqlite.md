...
**One writer at a time.** SQLite serialises writes, so tests inside a lane that expect real write concurrency will not behave as they would on a server engine. Enable WAL if the project does not already. A concurrency criterion proven only on SQLite remains unproven; if the ticket does not say how to prove it, stop for a human question through `docs/INBOX.md`.
