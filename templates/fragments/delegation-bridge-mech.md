...
The bridge reads each role's model, tools, and system prompt from `{{MECH_ROLE_DIR}}/<name>.md`; change a model by editing that file's `model:` line.

`reviewer`, `qa` and `decider` — the judgement roles — stay in the runner's native role location `{{RUNNER_ROLE_DIR}}/`. Use the harness's native subagent mechanism when available; otherwise dispatch them through the bridge. Return to a running one when the harness supports it; otherwise paste the prior report into the next dispatch.
